/**
 * Andor's Trail Content Generator v5
 *
...
function buildLoadResourcesXml(jsonFiles, tmxFiles) {
  const ARRAY_MAP = {
    'itemcategories_jasia.json':  'loadresource_itemcategories',
    'itemlist_jasia.json':          'loadresource_items',
    'actorconditions_jasia.json':   'loadresource_actorconditions',
    'monsterlist_jasia.json':       'loadresource_monsters',
    'droplists_jasia.json':         'loadresource_droplists',
    'conversationlist_jasia.json':  'loadresource_conversationlists',
    'questlist_jasia.json':         'loadresource_quests',
  };
  const groups = {};
...
function buildReadme(tmxFiles, jsonFiles) {
  return `# Andor's Trail Extended Content Pack v5
## Developer Reference
...
// ─── MAIN ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log("=== Andor's Trail Content Generator v5 ===");
  ...
  addJson('itemlist_jasia.json',         dedupedItems);
  addJson('monsterlist_jasia.json',      dedupedMonsters);
  addJson('droplists_jasia.json',        dedupedDL);
  addJson('conversationlist_jasia.json', dedupedConvs);
  addJson('questlist_jasia.json',        dedupedQuests);
  addJson('actorconditions_jasia.json',  dedupedACs);

  // Item categories — names are plain text per spec
  addJson('itemcategories_jasia.json', [
  ...
  const outPath = path.join(__dirname, '../andors_trail_content_v5.zip');
  ...
  console.log(`  Output:         andors_trail_content_v5.zip`);
}
