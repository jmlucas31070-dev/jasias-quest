/**
 * Andor's Trail Content Generator v4
 *
...
function buildZip(files) {
  ...
  let offset = 0;
  const dosDate = new Date(2026,4,9);
...
/**
 * iconID — AT format: "spritesheetname:linearIndex"
 * row/col are 1-based; cols is sprites per row in the sheet (default 16).
 * Result: "sheet:(row-1)*cols+(col-1)"
 */
function iconID(sheet, row, col, cols=16) {
  return `${sheet}:${(row-1)*cols+(col-1)}`;
}
...
/**
 * makeItem — builds a correct AT item JSON object.
 *
 * Per spec:
 *   - name         : plain text string (required)
 *   - description  : plain text string (optional, field name "description")
 *   - iconID       : "spritesheetname:linearIndex"
 *   - displaytype  : "ordinary"|"quest"|"extraordinary"|"legendary"|"rare"
 *   - hasManualPrice : 1 only when item has a manual price
 *   - baseMarketCost : only meaningful when hasManualPrice:1 (or as base value)
 *   - category     : references an itemcategory id
 *   - equipEffect  : object with increaseXxx fields for wearable items
 *   - useEffect    : timed effect for consumable items
 *   - hitEffect    : hit effect for weapon items
 *   - killEffect   : timed effect for weapon kills
 *
 * REMOVED vs v3: weight, charges, equipmentSlot (slot is via category), isQuestItem
 */
function makeItem({
  id, name, description='',
  iconSheet='items_necklaces', iconRow=1, iconCol=1,
...
/**
 * makeMonster — builds a correct AT NPC/monster JSON object.
 *
 * Per spec:
 *   - name         : plain text string (required)
 *   - iconID       : "spritesheetname:linearIndex"
 *   - maxHP, maxAP, moveCost, attackCost : top-level integer fields
 *   - attackChance, criticalSkill, criticalMultiplier : top-level
 *   - blockChance, damageResistance : top-level
 *   - attackDamage : {min, max} (replaces damagePotential)
 *   - spawnGroup   : string (faction group name)
 *   - faction      : string
 *   - phraseID     : dialogue id (replaces conversationID)
 *   - droplistID   : droplist id
 *
 * REMOVED vs v3: nested "stats" object, damagePotential
 */
function makeMonster({
  id, name,
  iconSheet='monsters_pack', iconRow=1, iconCol=1,
...
/**
 * makeDroplist — builds a correct AT droplist JSON object.
 *
 * Per spec:
 *   - chance : string representation of number (e.g. "100", "50", "0.1")
 *   - quantity : {min, max}
 *
 * FIXED vs v3: chance is now a string, not a number
 */
...
/**
 * makeConversation — builds a correct AT dialogue JSON object.
 *
 * Per spec:
 *   - message  : plain text string (required, optional if selector)
 *   - replies  : [{text, nextPhraseID, requires:[]}]
 *   - rewards  : [{rewardType, rewardID, value, mapName}]
 *
 * FIXED vs v3:
 *   - message is plain text (not @string/conv_xxx)
 *   - replies use nextPhraseID + requires[] (not conditions/scriptActions)
 *   - rewards use real AT rewardType/rewardID/value schema
 */
...
/**
 * makeQuest — builds a correct AT quest JSON object.
 *
 * Per spec:
 *   - name      : plain text string (required)
 *   - stages    : [{progress, logText, rewardExperience, finishesQuest}]
 *   - showInLog : 1 or 0
 *
 * FIXED vs v3: name and logText are plain text (not @string/ references)
 */
...
/**
 * makeActorCondition — builds a correct AT actor condition JSON object.
 *
 * Per spec:
 *   - name      : plain text string (required)
 *   - iconID    : "spritesheetname:linearIndex" (required)
 *   - category  : "spiritual"|"mental"|"physical"|"blood" (REQUIRED)
 *   - isPositive, isStacking : 1 or 0
 *   - abilityEffect : constant effect object
 *   - roundEffect, fullRoundEffect : timed effect objects
 *
 * FIXED vs v3: name is plain text; category is required; iconID format fixed
 */
function makeActorCondition({
  ...
  const ac = {
    ...
    name,
    iconID: iconID(iconSheet, iconRow, iconCol),
    category  // required field per spec
  };
...
function buildPickpocketSystem() {
  ...
  // Kill gold: 19 small gold coin items (1–19g each)
  const KILL_GOLD_ITEMS = range(19).map(g => {
    ...
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:2, iconCol:1+(g%8),
      baseMarketCost: g+1,
  ...
  // Pickpocket gold bags per thief level
  const PICKPOCKET_BAGS = PICKPOCKET_GOLD.map((gold, lvl) => {
    ...
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:2, iconCol:1+(lvl%8),
  ...
  // Kill droplist: random gold 1–19g
  ...
  // Per-target droplists (pickpocket success)
  range(200).forEach(i => {
  ...
  // Pickpocket action conversation — proper AT format
  conversations.push(makeConversation(
    'conv_pickpocket_action',
    'This person looks like they might have some coin on them.',
    [
      {text:'Pickpocket (50% success)',
       nextPhraseID:'X',
       requires:[reqFaction('thief',1)]},
      {text:'Pickpocket + Sneak (75% success)',
       nextPhraseID:'X',
       requires:[reqFaction('thief',6)]},
      {text:'Pickpocket + Hide (90% success)',
       nextPhraseID:'X',
       requires:[reqFaction('thief',9)]},
      {text:'Fight',
       nextPhraseID:'F'},
      {text:'Leave',
       nextPhraseID:'X'},
  ...
  range(200).forEach(i => {
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:1, iconCol:1+(i%8),
...
function buildTimekeeperSystem() {
  ...
  HOLIDAYS.forEach((h, hi) => {
    ...
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:9, iconCol:1+hi,
    ...
    holidayThemeGifts[h.id].forEach((name, i) => {
      items.push(makeItem({
        ...
        iconSheet:'items_necklaces', iconRow:10+hi, iconCol:1+i,
        baseMarketCost: 0, category:'miscellaneous'
      }));
    });
    ...
    holidayGoodGifts[h.id].forEach((name, i) => {
      items.push(makeItem({
        ...
        iconSheet:'items_necklaces', iconRow:16+hi, iconCol:1+i,
        baseMarketCost: 0, category:'miscellaneous'
      }));
    });
    ...
    // Actor conditions for holiday state tracking
  ...
  EVENTS.forEach((event, ei) => {
    eventThemeGifts[event].forEach((name, i) => {
      items.push(makeItem({
        ...
        iconSheet:'items_necklaces', iconRow:22+ei, iconCol:1+i,
        baseMarketCost:0, category:'miscellaneous'
      }));
    });
    actorconditions.push(makeActorCondition({
  ...
  // Time crystal (quest item, no useEffect — carry for 7 days)
  items.push(makeItem({
    ...
    displaytype:'quest',
    iconSheet:'items_necklaces', iconRow:26, iconCol:1,
    hasManualPrice:true, baseMarketCost:0, category:'quest'
  }));

  // Timekeeper conversation — proper AT dialogue format
  // Holiday reward dialogues chain off this one
  ...
  conversations.push(makeConversation(
    ...
    [reqInvRemove('item_time_crystal')].map ? // trick: inline requires as rewards
      [rQuestProgress('quest_timekeeper_crystal',20)] : []
  ));
  // Fix: rewards array for crystal done (requirement handled via reply requires)
  conversations[conversations.length-1].rewards = [rQuestProgress('quest_timekeeper_crystal',20)];
  conversations[conversations.length-1].replies[0].requires = [reqInvRemove('item_time_crystal',1)];

  // Holiday gift and quest chain dialogues
  HOLIDAYS.forEach((h,hi) => {
    conversations.push(makeConversation(
  ...
  REGIONS.forEach((region, ri) => {
    ...
    range(25).forEach(i => {
      ...
      items.push(makeItem({
        ...
        iconSheet:'items_necklaces', iconRow:3, iconCol:1+(i%8),
      ...
      monsters.push(makeMonster({
        ...
        iconSheet:'monsters_pack', iconRow:2+ri, iconCol:1+(i%8),
  ...
  REGIONS.forEach((region, ri) => {
    forageNames.forEach((base, i) => {
      items.push(makeItem({
        ...
        iconSheet:'items_necklaces', iconRow:5, iconCol:1+(i%8),
    ...
    // Forage result dialogue (gives a random forage item and sets cooldown)
    conversations.push(makeConversation(
...
function buildMiningContent() {
  ...
  ores.forEach((name, i) => {
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:6, iconCol:1+i,
    ...
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:7, iconCol:1+i,
    ...
    conversations.push(makeConversation(
      ...
      `You extract some ${name.toLowerCase()} from the wall.`,
      [{text:'Take it.', nextPhraseID:'X'}],
      [rDropList(`dl_mining_ore_${i}`)]
    ));
  });
  ...
  CRYSTALS.forEach((name, i) => {
    items.push(makeItem({
      ...
      description:`A beautiful ${name.toLowerCase()} found while mining.`,
      iconSheet:'items_necklaces', iconRow:8, iconCol:1+i,
      baseMarketCost:30+i*20, category:'ingredient'
    }));
...
function buildCraftingItems() {
  ...
  const CRAFTED_ARMOUR = [
    ...
    {id:'armour_shield_mithril',name:'Mithril shield',       dr:7, base:150, cat:'shield'},
  ];
  ...
  POTIONS.forEach(p => {
    ...
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:9, iconCol:1,
...
function buildGuildItems() {
  ...
  [['lockpick_basic','Basic lockpick','A basic lockpick. Breaks easily.',10],
   ...
   ['lockpick_master_pick','Master pick','The finest lockpick. Rarely breaks.',100],
    ...
    items.push(makeItem({id, name, description,
      iconSheet:'items_necklaces', iconRow:10, iconCol:1+i, baseMarketCost:base, category:'miscellaneous'
    }));
  });

  // Crafting deeds and special items
  ['small_home','mid_home','large_home','luxury_home'].forEach((t,i) => {
    items.push(makeItem({
      ...
      displaytype:'quest',
      iconSheet:'items_necklaces', iconRow:11, iconCol:1+i,
      hasManualPrice:true, baseMarketCost:0, category:'quest'
    }));
  });
  ...
  [['item_door_key','Door key','Opens a standard locked door.'],
   ['item_master_key','Master key','Opens any standard locked door.'],
    ...
    items.push(makeItem({id, name, description,
      iconSheet:'items_necklaces', iconRow:12, iconCol:1+i, baseMarketCost:20*(i+1), category:'key'
    }));
  });
  ...
  items.push(makeItem({
    ...
    displaytype:'quest',
    iconSheet:'items_necklaces', iconRow:12, iconCol:3,
    hasManualPrice:true, baseMarketCost:0, category:'quest'
  }));
  ...
  items.push(makeItem({
    ...
    iconSheet:'items_necklaces', iconRow:13, iconCol:1,
    baseMarketCost:5, category:'scroll'
  }));

  // Guild rank actor conditions
...
function buildGuildConversations() {
  ...
  // Door conversation — uses AT inventoryKeep/Remove requirements + nextPhraseID
  conversations.push(makeConversation(
    'conv_door',
    'A locked door bars your way.',
    [
      {text:'Open with key',
       nextPhraseID:'conv_door_open',
       requires:[reqInvRemove('item_door_key',1)]},
      {text:'Open with master key',
       nextPhraseID:'conv_door_open',
       requires:[reqInvKeep('item_master_key',1)]},
      {text:'Pick lock — basic lockpick (20% success)',
       nextPhraseID:'conv_door_pick_basic',
       requires:[reqInvKeep('lockpick_basic',1)]},
      {text:'Pick lock — iron lockpick (40% success)',
       nextPhraseID:'conv_door_pick_iron',
       requires:[reqInvKeep('lockpick_iron',1)]},
      {text:'Pick lock — steel lockpick (60% success)',
       nextPhraseID:'conv_door_pick_steel',
       requires:[reqInvKeep('lockpick_steel',1)]},
      {text:'Pick lock — master pick (80% success)',
       nextPhraseID:'conv_door_pick_master',
       requires:[reqInvKeep('lockpick_master_pick',1)]},
      {text:'Bash door (30% success)',
       nextPhraseID:'conv_door_bash'},
      {text:'Leave', nextPhraseID:'X'},
    ]
  ));
  conversations.push(makeConversation('conv_door_open','The door opens.',
    [{text:'Good.', nextPhraseID:'X'}]));
  ['basic','iron','steel','master'].forEach((tier,i) => {
  ...
  conversations.push(makeConversation('conv_door_bash',
    'You slam against the door.',
    [{text:'Try again.', nextPhraseID:'X'}]));
  ...
  conversations.push(makeConversation('conv_jail_lawyer',
    ...
    [
      {text:'Pay bail (100 gold)', nextPhraseID:'conv_jail_freed',
       requires:[reqActorCond('ac_bail_paid')]},
  ...
  // Guild master + seller conversations
  GUILDS.forEach((guild, gi) => {
    range(12).forEach(qi => {
    ...
    conversations.push(makeConversation(`conv_guildmaster_${guild}`,
      ...
      [
        {text:'Accept next quest', nextPhraseID:`conv_guild_quest_${guild}`,
         requires:[reqActorCond(`ac_guild_${guild}_rank_0`)]},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
    ...
    conversations.push(makeConversation(`conv_guild_seller_${guild}`,
      ...
      [
        {text:'Browse wares', nextPhraseID:'S',
         requires:[reqActorCond(`ac_guild_${guild}_rank_0`)]},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  ...
  // Mage/cleric/druid special desks
  ['mage','cleric','druid'].forEach(g => {
    conversations.push(makeConversation(`conv_desk_${g}`,
      ...
      [
        {text:'Browse scrolls', nextPhraseID:'S',
         requires:[reqActorCond(`ac_guild_${g}_rank_2`)]},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  });
  ...
  range(200).forEach(i => {
    conversations.push(makeConversation(`conv_home_loot_${i}`,
      ...
      [
        {text:'Search it', nextPhraseID:`conv_home_loot_${i}_result`,
         requires:[reqActorCond('ac_thief_guild_member')]},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  ...
  RACES.forEach((race, ri) => {
    ...
    range(25).forEach(i => {
      monsters.push(makeMonster({
        ...
        iconSheet:'monsters_pack', iconRow:4+ri, iconCol:1+(i%8),
    ...
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:4+ri, iconCol:9,
    ...
    items.push(makeItem({
      ...
      equipEffect:{
        addedConditions:[{condition:`ac_disguise_${race}`}]
      }
    }));
    actorconditions.push(makeActorCondition({
...
function buildQuestContent() {
  ...
  WITCH_INGREDIENTS.forEach((name, i) => {
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:15, iconCol:1+i,
      hasManualPrice:true, baseMarketCost:0, category:'quest'
    }));
  });
  ...
  SWAMP_MONSTERS.forEach((name, i) => {
    ...
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:7, iconCol:1+i,
  ...
  conversations.push(makeConversation('conv_witch',
    ...
    [
      {text:'Offer all 5 ingredients',
       nextPhraseID:'conv_witch_reward',
       requires:range(5).map(i=>reqInvRemove(`witch_ing_${i}`,1))},
  ...
  conversations.push(makeConversation('conv_witch_search',
    ...
    [
      {text:'Search (90% chance)', nextPhraseID:'conv_witch_search_result',
       requires:[reqNotActorCond('ac_witch_search_done')]},
      {text:'Leave', nextPhraseID:'X'},
    ]
  ));
  ...
  range(12).map(i => {
    items.push(makeItem({
      ...
      iconSheet:'items_necklaces', iconRow:16, iconCol:1+(i%8),
      hasManualPrice:true, baseMarketCost:0, category:'quest'
    }));
  });
  range(6).forEach(i => {
    ...
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:8, iconCol:1+i,
  ...
  ['drow_guard','drow_leader','drow_witch'].forEach((id, i) => {
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:9, iconCol:1+i,
    ...
    conversations.push(makeConversation(`conv_${id}`,
      ...
      [
        {text:'Accept quest', nextPhraseID:`conv_${id}_quest`},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  ...
  conversations.push(makeConversation('conv_drow_witch_spell',
    ...
    [
      {text:'Watch',
       nextPhraseID:'conv_drow_witch_spell_complete',
       requires:range(12).map(i=>reqInvRemove(`drow_ing_${i}`,1))},
      {text:'Leave', nextPhraseID:'X'},
    ]
  ));
  ...
  items.push(makeItem({
    ...
    iconSheet:'items_necklaces', iconRow:17, iconCol:1,
    hasManualPrice:true, baseMarketCost:0, category:'quest'
  }));
  ...
  DEMIGODS.forEach((name,i) => {
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:10, iconCol:1+i,
  ...
  monsters.push(makeMonster({
    ...
    iconSheet:'monsters_pack', iconRow:10, iconCol:8,
  ...
  conversations.push(makeConversation('conv_lloth',
    ...
    [
      {text:"Accept (take Lloth's ring)", nextPhraseID:'conv_lloth_accept'},
      {text:'Leave', nextPhraseID:'X'},
    ]
  ));
  ...
  DRAGON_TYPES.forEach((type, ti) => {
    DRAGON_AGES.forEach((age, ai) => {
      ...
      monsters.push(makeMonster({
        ...
        iconSheet:'monsters_pack', iconRow:12+ti, iconCol:1+ai,
      ...
      if (age==='adult') {
        ...
        conversations.push(makeConversation(`conv_dragon_${cid(type)}_adult`,
          ...
          [
            {text:'Accept quest', nextPhraseID:`conv_dragon_${cid(type)}_adult_quest`},
            {text:'Leave', nextPhraseID:'X'},
          ]
        ));
  ...
  ['ancient_platinum','ancient_chromatic'].forEach((did, i) => {
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:22, iconCol:1+i,
    ...
    conversations.push(makeConversation(`conv_dragon_${did}`,
      ...
      [
        {text:'Accept quest',
         nextPhraseID:`conv_dragon_${did}_accept`,
         requires:DRAGON_TYPES.map(t=>reqQuestProgress(`quest_dragon_${cid(t)}_adult`,10))},
        {text:'Return when you have completed all adult dragon quests.',
         nextPhraseID:'X'},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  ...
  items.push(makeItem({
    ...
    iconSheet:'items_necklaces', iconRow:18, iconCol:1,
    hasManualPrice:true, baseMarketCost:0, category:'quest'
  }));
  conversations.push(makeConversation('conv_sunny_ring_search',
    ...
    [
      {text:"Take the ring!", nextPhraseID:'conv_sunny_ring_found'},
      {text:'Leave it for now.', nextPhraseID:'X'},
    ]
  ));
  ...
  items.push(makeItem({
    ...
    iconSheet:'items_necklaces', iconRow:18, iconCol:2,
  ...
  MOON_COLORS.forEach((color, ci) => {
    range(12).forEach(j => {
      monsters.push(makeMonster({
        ...
        iconSheet:'monsters_pack', iconRow:14, iconCol:1+ci,
    ...
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:14, iconCol:8+ci%8,
    ...
    conversations.push(makeConversation(`conv_moon_elder_${cid(color)}`,
      ...
      [
        {text:'Accept quest', nextPhraseID:`conv_moon_quest_${cid(color)}`},
        {text:'Leave', nextPhraseID:'X'},
      ]
    ));
  ...
  CASTLE_NPCS.forEach(({id, name, conv, msg, qi}) => {
    if (!monsters.find(m=>m.id===id)) {
      monsters.push(makeMonster({
        id, name,
        iconSheet:'monsters_pack', iconRow:20, iconCol:1+(monsters.length%8),
...
function buildPokemonContent() {
  ...
  [['beast_orb_1','Spirit orb I','A basic spirit orb. 25% catch rate.',25,50],
   ...
   ['beast_orb_4','Spirit orb IV','A master spirit orb. 95% catch rate.',500,50],
    ...
    items.push(makeItem({
      id, name, description,
      iconSheet:'items_necklaces', iconRow:19, iconCol:iconC,
      baseMarketCost:base, category:'beastOrb'
    }));
  });

  // Beast actor conditions (owned, evolved, bred)
  POKEMON_REGIONS.forEach((region, ri) => {
    ...
    pool.forEach((base, bi) => {
      EVOLUTION_TIERS.forEach((tier, ti) => {
        ...
        monsters.push(makeMonster({
          ...
          iconSheet:'monsters_pack', iconRow:23+ri, iconCol:1+bi,
    ...
    monsters.push(makeMonster({
      ...
      iconSheet:'monsters_pack', iconRow:33, iconCol:1+ri,
    ...
    range(12).forEach(ti => {
      monsters.push(makeMonster({
        ...
        iconSheet:'monsters_pack', iconRow:34, iconCol:1+(ti%8),
    ...
    conversations.push(makeConversation(`conv_gym_leader_${region}`,
      ...
      [
        {text:'Challenge the gym leader!', nextPhraseID:'F'},
        {text:'Not ready yet.', nextPhraseID:'X'},
      ]
    ));
  ...
  monsters.push(makeMonster({
    ...
    iconSheet:'monsters_pack', iconRow:35, iconCol:1,
  ...
  // Beast scholar/professor/elder/breeder
  ['npc_beast_professor','npc_beast_scholar','npc_beast_elder','npc_beast_breeder'].forEach((id,i) => {
    ...
    monsters.push(makeMonster({
      id, name:names[i],
      iconSheet:'monsters_pack', iconRow:36, iconCol:1+i,
...
function buildStolenItems() {
  return {items:[]};  // Stolen items are now in buildGuildItems
}
...
function generateAllMaps() {
  ...
  // Misc conversations for map objects
  const extraConvs = [
...
function buildReadme(tmxFiles, jsonFiles) {
  return `# Andor's Trail Extended Content Pack v4
## Developer Reference
...
*Andor's Trail Extended Content Pack v4 — Generated May 2026*
`;
}

// ─── MAIN ─────────────────────────────────────────────────────────────────────
async function main() {
  console.log("=== Andor's Trail Content Generator v4 ===");
  ...
  const ALL_ITEMS = [
    ...
    ...pickpocket.items, ...timekeeper.items,
  ];
  const ALL_MONSTERS = [
  ...
  // JSON data files — flat AT arrays, plain text content, correct field names
  ...
  // Item categories — names are plain text per spec
  ...
  // XML resources — loadresources only (no strings.xml: content is in JSON)
  ...
  const outPath = path.join(__dirname, '../../andors_trail_content_v4.zip');
  ...
  console.log(`  Actor conds:    ${dedupedACs.length}`);
  console.log(`  ZIP size:       ${(zipBuf.length/1024).toFixed(0)}KB`);
  console.log(`  Output:         andors_trail_content_v4.zip`);
}
